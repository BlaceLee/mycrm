...
class Primary_menu(forms.ModelForm):
    class Meta:
        model = models.Primary_menu
        fields = '__all__'
        widgets = {
            'title': forms.TextInput(attrs={'class': 'form-control'}),
            'icon': forms.RadioSelect(choices=[[i[0], mark_safe(i[1])] for i in icon_list], attrs={'class': 'icon'}),
        }
...
def role_show(request):
    ...
    if request.method == "GET":
        return render(request, 'role_show.html', {'role_objs': role_objs})
...
# 角色编辑
def role_edit(request, rid=None):
...
def role_del(request, rid):
    role_obj = models.Role.objects.filter(pk=rid)
    role_obj.delete()
    return redirect('role_show')
...
def menu_list(request):
    ...
    if mid:
        permissions_objs = models.Permission.objects.filter(Q(primary_menus_id=mid) | Q(
            parent__primary_menus_id=mid)).values('id', 'title', 'url', 'primary_menus', 'parent_id',
                                                  'parent__title', 'primary_menus__title')
    else:
        mid = 0
        permissions_objs = models.Permission.objects.all().values('id', 'title', 'url', 'primary_menus', 'parent_id',
                                                                  'parent__title', 'primary_menus__title')

    primary_menu_objs = models.Primary_menu.objects.all()
    permissions_dic = {}
    for permission in permissions_objs:
        if not permission['parent_id']:
            permission['children'] = []
            permissions_dic[permission['id']] = permission
        else:
            permissions_dic[permission['parent_id']]['children'].append(permission)

    return render(request, 'menu_list.html', {'primary_menu_objs': primary_menu_objs,
                                              'permissions_dic': permissions_dic, 'mid': int(mid)},
                  )


# 展示各菜单对应所有的权限
def show_permissions(request, mid=None):
    primary_menu_objs = models.Primary_menu.objects.all()
    permissions_objs = models.Permission.objects.filter(Q(primary_menus_id=mid) | Q(
        parent__permission__primary_menus_id=mid))
    return render(request, 'menu_list.html', {'primary_menu_objs': primary_menu_objs,
                                              'permissions_objs': permissions_objs})


def menu_add_edit(request, mid=None):
    menu_objs = models.Primary_menu.objects.filter(pk=mid).first()
    if request.method == "GET":
        menu_forms = Primary_menu(instance=menu_objs)
        return render(request, 'forms.html', {'menu_forms': menu_forms})
    else:
        menu_forms = Primary_menu(request.POST, instance=menu_objs)
        if menu_forms.is_valid():
            menu_forms.save()
...
def menu_del(request, mid=None):
    models.Primary_menu.objects.filter(pk=mid).delete()
    return redirect('menu_list')
...
class MultiPermissionForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['title', 'url', 'primary_menus', 'parent', 'url_name']
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['parent'].choices = [(None, '---------')] + list(models.Permission.objects.filter(
            parent__isnull=True).exclude(primary_menus__isnull=True).values_list('id', 'title'))
...
def multi_permission(request):
    ...
    permissions_name_set = set([i.url_name for i in all_permissions])

    routes_dict = routes.get_all_url_dict(ignore_namespace_list=['admin', ])

    # 路由中的权限别名
    routes_name_set = set(routes_dict.keys())

    if request.method == "POST" and post_type == 'add':
        # print(post_type)
        add_formset = AddFormSet(request.POST)
        if add_formset.is_valid():
            # print(add_formset.cleaned_data)     # 获取提交的 所有数据
            permission_obj_list = [models.Permission(**i) for i in add_formset.cleaned_data]
            query_list = models.Permission.objects.bulk_create(permission_obj_list)  # 批量添加数据,返回每条记录组成的列表

            for query in query_list:
                permissions_name_set.add(query.url_name)

    add_name_set = routes_name_set - permissions_name_set
    add_formset = AddFormSet(initial=[row for name, row in routes_dict.items() if name in add_name_set])

    del_name_set = permissions_name_set - routes_name_set
    del_formset = FormSet(queryset=models.Permission.objects.filter(url_name__in=del_name_set))  # 数据库中有的路由而系统中没有的路由

    update_name_set = routes_name_set & permissions_name_set

    if request.method == "POST" and post_type == 'update':
        update_formset = FormSet(request.POST)
        if update_formset.is_valid():
            # print(">>>", update_formset.cleaned_data)
            update_formset.save()
    ...
    if request.method == "POST" and post_type == 'del':
        ...
        if del_formset.is_valid():
            # print(del_formset.cleaned_data)
            del_name_list = [i['url_name'] for i in del_formset.cleaned_data]
            models.Permission.objects.filter(url_name__in=del_name_list).delete()
            for u_name in del_name_list:
                permissions_name_set.remove(u_name)
...
def permission_show(request):
    ...
    post_type = request.GET.get('type')
    if request.method == "POST" and post_type == 'save_role':
        user = models.User.objects.filter(pk=uid).first()
        role_list = request.POST.getlist('has_role')
        user.roles.set(role_list)

    if request.method == "POST" and post_type == 'save_permission' and rid:
        role = models.Role.objects.filter(pk=rid).first()
        permission_list = request.POST.getlist('has_permission')
        role.permissions.set(permission_list)

    user_objs = models.User.objects.all()
    user_has_roles = models.User.objects.filter(pk=uid).values('id', 'roles')
    user_role_list = [i['roles'] for i in user_has_roles]
    role_objs = models.Role.objects.all()

    if rid:
        role_has_permissions = models.Role.objects.filter(pk=rid).values('id', 'permissions')
    ...
    user_role_permissions_list = [i['permissions'] for i in role_has_permissions]

    # 权限
    menu_dict = {}
    menu_list = models.Primary_menu.objects.all().values('id', 'title')
    ...
    # 二级菜单
    permission_dict = {}
    permissions_list = models.Permission.objects.filter(primary_menus__isnull=False).values(
        'id',
        'title',
        'primary_menus_id')
    ...
    for permission in permissions_list:
        ...
        menu_dict[permission['primary_menus_id']]['children'].append(permission)

    # 子权限   所有父权限不为空
    sub_permissions = models.Permission.objects.filter(parent__isnull=False).exclude(
        primary_menus__isnull=False).values('id', 'title', 'parent_id')

    for sub_permission in sub_permissions:
        permission_dict[sub_permission['parent_id']]['children'].append(sub_permission)

    return render(request, 'permissions.html',
                  {'user_objs': user_objs,
                   'role_objs': role_objs,
                   'user_role_list': user_role_list,
                   'user_role_permissions_list': user_role_permissions_list,
                   'menu_dict': menu_dict,
                   'uid': uid,
                   'rid': rid})
